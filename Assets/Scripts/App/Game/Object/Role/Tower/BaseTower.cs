...
namespace App.Game.Generic.BaseObject
{
    ...
    public abstract class BaseTower : MonoBehaviour, ITower
    {
        ...
        {
            // 有集火目标直接锁定
            var monster = Spawner.GetCollectingFiresTarget();
            if (monster != null)
            {
                var distance = Vector3.Distance(transform.position, monster.Transform.position);

                // 处于攻击范围
                if (distance < data.attackRangesList[level] && !monster.IsDead) target = monster;
            }
        }
        ...
        protected void FindTargets()
        {
            ...
            var monsters = Spawner.GetAllMonsters();
            targetDic.Clear();
            // 获取距离list
            foreach (var m in monsters)
            {
                ...
                // 处于攻击范围
                if (distance < data.attackRangesList[level] && !m.IsDead) targetDic[distance] = m;
            }

            // 选择最近目标
            if (targetDic.Count > 0) target = targetDic[targetDic.Keys.Min()];
        }
