...
namespace App.Game.Object.Map
{
    ...
    public class Map : MonoBehaviour
    {
        ...
        private static readonly List<Cell> cellsList = new List<Cell>(); // 所有格子

        private readonly List<TowerData> towerDataList = new List<TowerData>(); // 塔数据
        ...
        {
            var x = (worldPos.x + mapWidth / 2f) / (mapWidth / ColumnNum);
            var y = (worldPos.y + mapHeight / 2f) / (mapHeight / RowNum);
        ...
        public void Init(MapData mapData)
        {
            ...
            // 获取允许使用的塔数据
            towerDataList.Clear();
            for (var i = 0; i < mapData.towerTypeList.Count; i++) towerDataList.Add(mapData.GetTowerData(i));
        }
        ...
        {
            // 加载放塔点覆盖空数据的格子
            for (var i = 0; i < nowMapData.towerPosList.Count; i++)
        ...
        public void OnMouseDown()
        {
            ...
            // 射线检测判断是否被UI遮挡
            var gr = UIManager.Instance.canvas.GetComponent<GraphicRaycaster>();
            ...
            {
                // 显示禁止建造图标
                GameFacade.Instance.SendNotification(NotificationName.UI.SHOW_CANT_BUILT_ICON, GetCellCenterPos(cell));
            }
        }
        ...
        {
            var towerData = tower.GetData();
            var level = tower.GetLevel();

            var body = new UpGradeTowerArgsBody();
        ...
        {
            var towersDataDic = new Dictionary<TowerData, Sprite>();
            // 获取当前关卡可创建塔的所有Icons
            for (var i = 0; i < towerDataList.Count; i++)
                // 判断是否够钱, 获取0级的Icon
