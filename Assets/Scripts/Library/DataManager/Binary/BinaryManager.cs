...
namespace Library
{
    public class BinaryManager : BaseSingleton<BinaryManager>
    {
        ...
        {
            // 不存在该类的二进制文件返回默认值对象
            if (!File.Exists(BINARYFILE_PATH + fileName)) return new T();

            T obj;

            try
            {
                using (var fileStream = File.Open(BINARYFILE_PATH + fileName, FileMode.Open, FileAccess.Read))
                {
        ...
        {
            // 不存在该类的二进制文件则创建
            if (!File.Exists(BINARYFILE_PATH + fileName)) File.Create(BINARYFILE_PATH + fileName).Close();

            using (var fileStream = File.Open(BINARYFILE_PATH + fileName, FileMode.Open, FileAccess.Write))
            {
