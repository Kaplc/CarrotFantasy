...
public interface IAddressablesManager
{
    ...
    void LoadAssetAsync(Type type, Action<object> callBack, params string[] keys);

    void LoadAssetsAsync<T>(Addressables.MergeMode mode, Action<IList<T>> callBack, params string[] keys) where T : class;

    void LoadAssetsAsync(Type type, Addressables.MergeMode mode, Action<IList<object>> callBack, params string[] keys);
